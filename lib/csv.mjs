...
export function problems(p, rows, collections) {
  ...
  for (const key of [
    ...
    if (!String(p[key] ?? "").trim()) issues.push(`${key} is required`);
  ...
  if (!["USD", "EUR", "GBP", "CAD", "AUD"].includes(p.currency))
    issues.push("Choose a supported currency");
  if (!["reference", "asking"].includes(p.price_kind))
    issues.push("Choose reference or asking price");
