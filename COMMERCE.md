...
The current customer flow is product or private offer → quote request → operator proposal → customer acceptance. No on-site payment, shipping booking or paid-order creation endpoint is active. `/api/catalog` explicitly reports `checkout: false` and `delivery: false`.
...
3. Add inventory reservation to the implemented server-authoritative, versioned quote acceptance flow. Convert accepted proposal totals into immutable order snapshots; never trust customer-submitted prices. Define reservation expiry and stock release on abandoned checkout.
